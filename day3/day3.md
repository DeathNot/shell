...
Li is not empty!
```
在比较字符串的时候，比较测试会将所有的标点和大小写情况都考虑在内；在比较字符串大小时，大于号和小于号必须转义，并且大于和小于顺序和sort命令所采用的不同。比较测试使用的是标准ASCII顺序，而sort使用的是系统的本地化语言设置中定义的排序顺序。对于英语，本地化设置指定了在顺序排序中小写字母出现在大写字母前。一个未被定义的变量字符串长度任为0。
### 2.4.3 文件比较
